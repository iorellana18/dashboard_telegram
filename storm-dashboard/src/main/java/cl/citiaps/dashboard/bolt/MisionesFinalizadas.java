...
public class MisionesFinalizadas implements IRichBolt {

	private static final long serialVersionUID = 7784329420249780555L;

	private static Logger logger = LoggerFactory.getLogger(MisionesFinalizadas.class);
	...
	public MisionesFinalizadas(long timeDelay, long emitTimeframe) {
	...
	public void execute(Tuple tuple) {
		...
		if (log.getAccion().equals("FINISH_MISSION") && log.getTipoUsuario().equals("COORDINATOR")) {
			rateMision++;
	...
	private class EmitTask extends TimerTask {
		...
		public void run() {
			...
			Count acum = new Count("misionesTerminadasCount", ParseDate.parse(timestampCurrent), count);
			Count rate = new Count("misionesTerminadasRate", ParseDate.parse(timestampCurrent), rateMision);
